...
.controller('EscenarioCtrl', function($scope, $ionicPlatform, $http, $cordovaDeciveMotion, $cordovaDeviceOrientation, ClientService, Formas) {
...
.controller('DebugCtrl', function($scope, $ionicPlatform, $http, $cordovaDeviceMotion, $cordovaDeviceOrientation) {
	...
	watch1=$cordovaDeviceMotion.watchAcceleration({frequency:1000}).subscribe(result => {document.getElementById("x1").innerHTML=JSON.stringify(result)});

	watch2=$cordovaDeviceOrientation.watchHeading({frequency:1000}).subscribe(result => {
...
//	var intervalID2 = setInterval(function() {$http.post(url+"/"+JSON.stringify(pos),"")}, 100);
});
});
